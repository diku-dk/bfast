...
class BFASTMonitorOpenCL(BFASTMonitorBase):
    ...
    def __init__(self,
        ...
        if k not in k_valid:
            raise ValueError("Current implementation can only handle the following values for k: {}".
                             format(k_valid))
    ...
    def _fit_chunks(self, data, dates, n_chunks=10, nan_value=0):
        ...
        for chunk_idx in range(n_chunks):
            ...
            self.logger.info("Processing chunk index {}/{}".format(chunk_idx + 1, n_chunks))
            ...
            if self.verbosity > 0:
                ...
                self.logger.info("- runtime for chunk normalization:\t\t\t{}".format(end - start))
            ...
            if self.verbosity > 0:
                ...
                self.logger.info("- runtime for fitting chunk:\t\t\t\t{}".format(end - start))

            end_chunk = time.time()
            self.logger.info("- all computations have taken:\t\t\t\t{}".format(end_chunk - start_chunk))
    ...
    def _fit_single_preprocess(self, data, dates, nan_value):
        ...
        if self.verbosity > 0:
            ...
            self.logger.info("--- runtime for data initialization:\t\t{}".format(end - start))
        ...
        if self.verbosity > 0:
            ...
            self.logger.info("--- runtime for data transfer (host->device):\t{}".format(end - start))
        ...
        if self.verbosity > 0:
            ...
            self.logger.info("--- runtime for data preprocessing:\t\t{}".format(end - start))
    ...
    def _fit_single_kernel(self, y_cl, mapped_indices_cl):
        ...
        if self.verbosity > 0:
            ...
            self.logger.info("--- runtime for kernel execution:\t\t{}".format(end - start))
    ...
    def _fit_single_postprocessing(self, results, oshape):
        ...
        if self.verbosity > 0:
            ...
            self.logger.info("--- runtime for data transfer (device->host):\t{}".format(end - start))

    def _merge_results(self, results):
        final_results = {}
        while(len(results) > 0):
            self.logger.debug("Length of list to be merged: {}".format(len(results)))
