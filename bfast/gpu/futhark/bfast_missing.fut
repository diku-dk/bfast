...
let bfast [N] (f: f32) (k: i32) (n: i32)
  ...
  -- line 4: error
  let y_error_all = map3 (\flg ye yep -> if flg then (ye-yep) else f32.nan) flgs y y_pred -- [N]
...
--  let (_, y_error) = unzip <| filter (\(flg,_) -> flg) (zip flgs y_error_all)
--  let (_, val_inds)= unzip <| filter (\(flg,_) -> flg) (zip flgs (iota (N-n)))
  let (_,y_error,val_inds)  =
      unzip3 <| unsafe filter (\(flg,_,i) -> flg) (zip3 flgs y_error_all (iota N))
  ...
  let val_inds' = unsafe drop ns val_inds |> map (\x -> x-n)
  let full_MO = unsafe scatter (replicate (N-n) f32.nan) val_inds' MO
  ...
  let breaks = map2 (\m b -> if (f32.isnan m) || (f32.isnan b)
                             then false else (f32.abs m) > b --used to be nan instead of 0.0
                    ) full_MO BOUND
                    
  in  (full_MO, BOUND, breaks, y_error_all, y_pred)
